class OpsAddProSubjectsNotifyTriggers < ActiveRecord::Migration[7.1]
  def up
  ...
  def down
    Trigger.where(name: [
      'ops - preposielanie nových podnetov PRO zodpovedným subjektom',
      'ops - preposielanie upravených podnetov PRO zodpovedným subjektom',
      'ops - preposielanie nových komentárov PRO zodpovedným subjektom'
    ]).destroy_all
  end
end
